...
console.log(url);
  ...
  var load_all = function(url){
    d3.csv("json/CrossCorrelationMatrix_JsonFiles/"+url+".csv", function(data){
...
var main = function(corr, label_col, label_row){
  ...
  var body = d3.select('div#display');
...
//    .style("opacity", 1e-6);
 ...
 /*   tick_col.transition()
  ...
  var matrix = svg.append('g')
      .attr('class','matrix')
      .attr('transform', 'translate(' + (label_space -140) + ',' + (label_space + 10) + ')')
      .attr('width',scale(0.5))
  ...
  tick_row = svg.append('g')
      .attr('class','ticks')
      .attr('transform', 'translate(' + (label_space - 150) + ',' + (label_space+10) + ')')
      .selectAll('text.tick')
      .data(label_row);
